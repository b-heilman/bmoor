...
describe('bmoor.array', function () {
	...
	it('should support the matching of two arrays', function () {
		var t = bmoor.array.compare(
			[{val: 1}, {val: 3}, {val: 9}, {val: 5}, {val: 6}],
			[{val: 2}, {val: 4}, {val: 5}, {val: 1}, {val: 9}, {val: 10}],
			function (a, b) {
				return a.val - b.val;
			}
		);

		expect(t.left).to.deep.equal([{val: 3}, {val: 6}]);

		expect(t.intersection.left).to.deep.equal([{val: 1}, {val: 5}, {val: 9}]);

		expect(t.intersection.right).to.deep.equal([{val: 1}, {val: 5}, {val: 9}]);

		expect(t.right).to.deep.equal([{val: 2}, {val: 4}, {val: 10}]);
	});
	...
	describe('::intersection', function () {
		it('should properly check intersection of two arrays', function () {
			expect(
				bmoor.array.intersection([1, 2, 3, 4, 5, 6], [2, 3, 4, 7, 8])
			).to.deep.equal([2, 3, 4]);
		});
	});
	...
	describe('::difference', function () {
		it('should properly check difference of two arrays', function () {
			expect(
				bmoor.array.difference([1, 2, 3, 4, 5, 6], [2, 3, 4, 7, 8])
			).to.deep.equal([1, 5, 6]);
		});
	});
